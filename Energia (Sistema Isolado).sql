--drop function calcular_emissoes_compra_energia_sistema_isolado
CREATE OR REPLACE FUNCTION calcular_emissoes_compra_energia_sistema_isolado(
...
BEGIN
    -- Verificar se a categoria de emissão é "COMPRA DE ENERGIA ELETRICA - SISTEMA ISOLADO"
    IF categoria_emissao_input = 'COMPRA DE ENERGIA ELETRICA - SISTEMA ISOLADO' THEN
        ...
        EXECUTE format(
            'SELECT co2 FROM fator_emissao_bio_ch4 WHERE biocombustivel = %L',
        ...
        SELECT 
            ...
            combustivel_fossil_utilizado;
    ELSE
        -- Retornar NULLs se a categoria de emissão não for "COMBUSTÃO ESTACIONÁRIA"
        RETURN QUERY SELECT NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL;
    END IF;
END;
$$ LANGUAGE plpgsql;
